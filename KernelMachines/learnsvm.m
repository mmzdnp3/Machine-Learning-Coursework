...
%           @(A,B) (A*B' + c).^d
	...
	function nch = update(i,j)
        ...
        alpha_newi = ((-1/2) * ((alpha(i)+alpha(j)*Y(j)*Y(i))*(K(j,i)+K(i,j)-2*K(j,j)) + 2*(tilde_a'*q*[1; -Y(i)/Y(j)])) + 1 -Y(i)/Y(j))/(K(i,i)-2*K(j,i)+K(j,j));
        
        if Y(j) == 1
            if Y(i) == 1
                lowerbound = max(0,alpha(i) + (-C*Y(j)+alpha(j)*Y(j))/Y(i));
                upperbound = min(C,alpha(i)+alpha(j)*Y(j)/Y(i));
            else
                lowerbound = max(0,alpha(i)+alpha(j)*Y(j)/Y(i));
                upperbound = min(C,alpha(i) + (-C*Y(j)+alpha(j)*Y(j))/Y(i));
            end
        else
            if Y(i) ==1
                lowerbound = max(0,alpha(i)+alpha(j)*Y(j)/Y(i));
        ...
        if alpha_newi > upperbound
            alpha_newi = upperbound;
        end
        if alpha_newi < lowerbound;
            alpha_newi = lowerbound;
        end
 
        alpha_newj = (alpha(i)*Y(i) + alpha(j)*Y(j) - alpha_newi*Y(i))/Y(j);
        ...
        if abs(alpha(i) - alpha_newi) > (10^-10 * (alpha(i) + alpha_newi + 10^-10))
            alpha(i) = alpha_newi;
            alpha(j) = alpha_newj;
            b = findb(K*(alpha.*Y),Y);
            nch=1;
